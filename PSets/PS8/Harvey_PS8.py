...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 00:14:33 2017
...
eps = np.random.normal(0, sigma_eps, size=(num_draws))
...
#Here we plot the distribution of z and print the theoretical z
...
### Simulate transition matrix (enter Neo)...
...
# Oh the probabilities are a changing
pi = np.empty((N, N))
for i in range(N):
    for j in range(N):
...
def MKTclear(w):
    ...
    K = np.zeros(int(abs(numb) * abs(dens)))
    ...
    # Make ready for value funtion iterations
    VFtol = 1e-6
    VFdist = 7.0
    VFmaxiter = 3000
    ...
    for i in range(sizez):
        for j in range(sizek):
          ...
          ((kgrid[j] ** alpha_k) ** (1 / (1 - alpha_l))) * (z_grid[i] ** 
          (1/(1 - alpha_l))))


    ### Find Stationary Distribution although finding that which does not move
    #should be easy...
    ...
    while SDdist > SDtol and SDmaxiter > SDiter:
        HGamma = SD(PF, pi, Gamma, sizez, sizek)
    ...
    # labor demand
    optAggLD = Agg(optLD, Gamma)

    # Investment
    optAggI = Agg(optI, Gamma)
    ...
    # Output
    optY = np.multiply(np.multiply((optLD) ** alpha_l, kgrid ** alpha_k),
                       np.transpose([z_grid]))
    optAggY = Agg(optY, Gamma)

    # Consumption
    optCON = optAggY - optAggI - optAADJC


    ### Find labor supply
    optAggLS = w/(h * optCON)

    mkclear = abs(optAggLS - optAggLD)
    ...
    ax.set_xlabel(r'Log Productivity Shock')
    ax.set_ylabel(r'Capital Stock')
    ax.set_zlabel(r'Density')
    fig.savefig('panic_station.png', transparent=False, dpi=80, 
                bbox_inches="tight")
    ...
    fig.savefig('policy.png', transparent=False, dpi=80, 
                bbox_inches="tight")
    
    return mkclear
    
#Here we "decorate" two functions we will use with numba...
@numba.jit
...
@numba.jit
def SD(PF, pi, Gamma, sizez, sizek):
...
# Call on the minimizer
# Minimize with (truncated) Newton's method (called Newton Conjugate Gradient method)
wage_initial = w
GE_results = opt.minimize(MKTclear, wage_initial, method='Nelder-Mead', 
